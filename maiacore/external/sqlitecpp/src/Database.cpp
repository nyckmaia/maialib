...
void Database::Deleter::operator()(sqlite3* apSQLite) {
    const int ret = sqlite3_close(
        apSQLite);  // Calling sqlite3_close() with a nullptr argument is a harmless no-op.
...
// Shortcut to execute one or multiple SQL statements without results (UPDATE, INSERT, ALTER,
// COMMIT, CREATE...). Return the number of changes.
...
Column Database::execAndGet(const char* apQuery) {
    ...
    (void)query
        .executeStep();  // Can return false if no result, which will throw next line in getColumn()
...
void Database::loadExtension(const char* apExtensionName, const char* apEntryPointName) {
    ...
    // Security warning:
    // It is recommended that the SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION method be used to enable
    // only this interface. The use of the sqlite3_enable_load_extension() interface should be
    // avoided to keep the SQL load_extension() disabled and prevent SQL injections from giving
    // attackers access to extension loading capabilities. (NOTE: not using nullptr: cannot pass
    // object of non-POD type 'std::__1::nullptr_t' through variadic function)
...
void Database::backup(const char* apFilename, BackupType aType) {
    ...
    // For a 'Save' operation, data is copied from the current Database to the other. A 'Load' is
    // the reverse.
