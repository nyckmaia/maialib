...
// Those public constants enable most usages of SQLiteCpp without including <sqlite3.h> in the
// client application.
...
/// With OPEN_READWRITE: The database is opened for reading and writing, and is created if it does
/// not already exist.
...
/// Database filename is not allowed to be a symbolic link (Note: only since SQlite 3.31.0 from
/// 2020-01-22)
 ...
 */
class Database {
    friend class Statement;  // Give Statement constructor access to the mSQLitePtr Connection
                             // Handle
   ...
   public:
     ...
     *
     * @param[in] apFilename        UTF-8 path/uri to the database file ("filename" sqlite3
     * parameter)
     * @param[in] aFlags SQLite::OPEN_READONLY/SQLite::OPEN_READWRITE/SQLite::OPEN_CREATE...
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before returning SQLITE_BUSY (see
     * setBusyTimeout())
     ...
     * Exception is thrown in case of error, then the Database object is NOT constructed.
     *
     * @param[in] aFilename         UTF-8 path/uri to the database file ("filename" sqlite3
     * parameter)
     * @param[in] aFlags SQLite::OPEN_READONLY/SQLite::OPEN_READWRITE/SQLite::OPEN_CREATE...
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before returning SQLITE_BUSY (see
     * setBusyTimeout())
     * @param[in] aVfs              UTF-8 name of custom VFS to use, or empty string for sqlite3
     * default
     *
     * @throw SQLite::Exception in case of error
     */
    ...
    // Deleter functor to use with smart pointers to close the SQLite database connection in an RAII
    // fashion.
    struct Deleter {
     ...
     *
     *  This is useful in multithreaded program to handle case where a table is locked for writing
     * by a thread. Any other thread cannot access the table and will receive a SQLITE_BUSY error:
     *  setting a timeout will wait and retry up to the time specified before returning this
     * SQLITE_BUSY error. Reading the value of timeout for current connection can be done with SQL
     * query "PRAGMA busy_timeout;". Default busy timeout is 0ms.
     *
    ...
    /**
     * @brief Shortcut to execute one or multiple statements without results. Return the number of
     * changes.
     *
     ...
     * @see Statement::exec() to handle precompiled statements (for better performances) without
     * results
     ...
     *
     * @return number of rows modified by the *last* INSERT, UPDATE or DELETE statement (beware of
     * multiple statements)
     * @warning undefined for CREATE or DROP table: returns the value of a previous INSERT, UPDATE
     * or DELETE statement.
     *
     * @throw SQLite::Exception in case of error
     */
     ...
     * @return the sqlite result code.
     */
    int tryExec(const std::string& aQueries) noexcept { return tryExec(aQueries.c_str()); }
     ...
     * This should be used only for non reusable queries (else you should use a Statement with
     * bind()). This should be used only for queries with expected results (else an exception is
     * fired).
     *
     ...
     * @throw SQLite::Exception in case of error
     */
    Column execAndGet(const std::string& aQuery) { return execAndGet(aQuery.c_str()); }
    ...
    /**
     * @brief Get the rowid of the most recent successful INSERT into the database from the current
     * connection.
     *
    ...
    /// Return UTF-8 encoded English language explanation of the most recent failed API call (if
    /// any).
     ...
     *
     * This is often needed to mix this wrapper with other libraries or for advance usage not
     * supported by SQLiteCpp.
     */
     ...
     * @param[in] abDeterministic Optimize for deterministic functions (most are). A random number
     * generator is not.
     * @param[in] apApp         Arbitrary pointer of user data, accessible with sqlite3_user_data().
     * @param[in] apFunc        Pointer to a C-function to implement a scalar SQL function (apStep &
     * apFinal nullptr)
     * @param[in] apStep        Pointer to a C-function to implement an aggregate SQL function
     * (apFunc nullptr)
     * @param[in] apFinal       Pointer to a C-function to implement an aggregate SQL function
     * (apFunc nullptr)
     * @param[in] apDestroy     If not nullptr, then it is the destructor for the application data
     * pointer.
     *
     * @throw SQLite::Exception in case of error
     */
     ...
     *  Open encrypted database -> call db.key("secret") -> call db.rekey("newsecret") -> change
     * key, database ready Open encrypted database -> call db.key("secret") -> call db.rekey("") ->
     * decrypted database, database ready
     *
    ...
    /**
     * @brief Check if aRet equal SQLITE_OK, else throw a SQLite::Exception with the SQLite error
     * message
     */
