...
TEST(freq2midiNote, centsValues) {
  // Obs: In this case the (non zero) cents were calculated separately (with a
  // calculator) Enter with a given frequency (not in the table) and calculate
  // the deviation (in CENTS) from the closest  frequency (up or down) from the
  // table. float midi_50cents_up = 1.029302; // this factor increase 50 cents
  // (f2= a*f1 ) ==> this makes round up the value of MIDI note float
  // midi_50cents_down = 0.971532; // this factor decrease 50 cents on the value
  // of MidiNumber ==> makes round down the value of MIDI note
...
TEST(midiNote2pitch, twelveTonesOctave4) {
  ...
  // EXPECT_EQ(testing::internal::GetCapturedStderr(), "[maiacore] The MIDI Note
  // '60' cannot be wrote using 'b' accident type\n");
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '60' cannot
    // be wrote using 'b' accident type\nSource File: helper.cpp - Line
    // 89\nFunction: static const std::string Helper::midiNote2pitch(int, const
    // std::string&)"));
  } catch (...) {
  ...
  try {
    ...
    // FAIL() << "Expected [maiacore] The MIDI Note '60' cannot be wrote using
    // 'x' accident type";
  } catch (std::runtime_error const& err) {
    // EXPECT_EQ(err.what(), "...");
  } catch (...) {
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '61' cannot
    // be wrote using 'bb' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '62' cannot
    // be wrote using 'b' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '63' cannot
    // be wrote using 'x' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '64' cannot
    // be wrote using 'bb' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '65' cannot
    // be wrote using 'b' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '66' cannot
    // be wrote using 'bb' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '67' cannot
    // be wrote using 'b' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '68' cannot
    // be wrote using 'bb' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '69' cannot
    // be wrote using 'b' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '70' cannot
    // be wrote using 'x' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '71' cannot
    // be wrote using 'bb' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
  ...
  try {
    ...
    // EXPECT_EQ(err.what(), std::string("[maiacore] The MIDI Note '72' cannot
    // be wrote using 'b' accident type"));
  } catch (...) {
    FAIL() << "Expected std::runtime_error";
  }
...
TEST(ticks2noteType, basicNoteTypes) {
  ...
  EXPECT_EQ(Helper::ticks2noteType(57344, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::MAXIMA_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(49152, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::MAXIMA_DOT);
  EXPECT_EQ(Helper::ticks2noteType(32768, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::MAXIMA);
  EXPECT_EQ(Helper::ticks2noteType(28672, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::LONG_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(24576, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::LONG_DOT);
  EXPECT_EQ(Helper::ticks2noteType(16384, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::LONG);
  EXPECT_EQ(Helper::ticks2noteType(14336, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::BREVE_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(12288, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::BREVE_DOT);
  EXPECT_EQ(Helper::ticks2noteType(8192, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::BREVE);
  EXPECT_EQ(Helper::ticks2noteType(7168, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::WHOLE_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(6144, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::WHOLE_DOT);
  EXPECT_EQ(Helper::ticks2noteType(4096, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::WHOLE);
  EXPECT_EQ(Helper::ticks2noteType(3584, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::HALF_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(3072, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::HALF_DOT);
  EXPECT_EQ(Helper::ticks2noteType(2048, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::HALF);
  EXPECT_EQ(Helper::ticks2noteType(1792, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::QUARTER_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(1536, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::QUARTER_DOT);
  EXPECT_EQ(Helper::ticks2noteType(1024, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::QUARTER);
  EXPECT_EQ(Helper::ticks2noteType(896, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::EIGHTH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(768, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::EIGHTH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(512, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::EIGHTH);
  EXPECT_EQ(Helper::ticks2noteType(448, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N16TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(384, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N16TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(256, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N16TH);
  EXPECT_EQ(Helper::ticks2noteType(224, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N32ND_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(192, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N32ND_DOT);
  EXPECT_EQ(Helper::ticks2noteType(128, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N32ND);
  EXPECT_EQ(Helper::ticks2noteType(112, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N64TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(96, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N64TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(64, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N64TH);
  EXPECT_EQ(Helper::ticks2noteType(56, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N128TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(48, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N128TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(32, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N128TH);
  EXPECT_EQ(Helper::ticks2noteType(28, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N256TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(24, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N256TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(16, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N256TH);
  EXPECT_EQ(Helper::ticks2noteType(14, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N512TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(12, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N512TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(8, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N512TH);
  EXPECT_EQ(Helper::ticks2noteType(7, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N1024TH_DOT_DOT);
  EXPECT_EQ(Helper::ticks2noteType(6, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N1024TH_DOT);
  EXPECT_EQ(Helper::ticks2noteType(4, divisionsPerQuarterNote).first,
            MUSIC_XML::NOTE_TYPE::N1024TH);
}
