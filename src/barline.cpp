...
std::string Barline::toXML(const int identSize) const
{
    ...
    if (getLocation().empty()) {
        ...
        xml.append(Helper::generateIdentation(3, identSize) + "<barline location=\"" + getLocation() + "\">\n");
    }

    xml.append(Helper::generateIdentation(4, identSize) + "<bar-style>" + getBarStyle() + "</bar-style>\n");

    if (!getDirection().empty()) {
        xml.append(Helper::generateIdentation(4, identSize) + "<repeat direction=\"" + getDirection() + "\"/>\n");
    }
